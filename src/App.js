    ...
    <div className="App">
      <div className="header">
        ...
        <div id="introduction-wrapper">
          ...
          <div className="intro-text">
            This are the values calculated in the calibration phase. On the X axis are representd the odds. To each odd is associated a group of matches, specifically all the matches for which the algorithm identified the aforementioned predicted odd as the most probable one for the Over 2.5 event. The grey line corresponds to the ideal distribution, which is obtained when the frequency of an event happening is exactly equal to <code>100/xQuote</code>. The blue line represents the actual distribution, which means the Y axis contains the frequency of the event happening in each match group. For example regarding the 1.4 odd, the ideal frequency value is <code>100/1.4 = 71.5%</code> whilst the actual frequency is calculated as the total number of matches in the 1.4 group ending with an Over 2.5 and the total number of matches in the 1.4 group: in this case <code>61%</code>. Generally speaking, the distance between the grey and the blue line can be considered the calibration error and should be minimised. That said, there is a difference between having an error generated by overstimating or understimating the odd. As we can see in the left part of the graph, the error is highlighted in red because this is where money is actually lost. Concentrating again on the 1.4 odd, we can see that the frequency of the event happening is much lower than what we would expect. This means that the algorithm will suggest betting more often than it should, resulting in losing money, even considering the value threshold. Let's say we bet 1$ on 100 matches in which the bookie offers an odd of 1.5 while the algorithm predicts 1.4. Ideally, we would win 71.5% of the times, meaning we will get back roughly <code>71 * 1.5 = 106.5$</code>, with a profit of <code>6.5$</code>. Considering the real frequency though, we will win only <code>61 * 1.5 = 91.5$</code>, corresponding to a loss of <code>8.5$</code>. The right side of the graph gives us a completely different situation. Even if this can still be considered an error, in this case we are not losing money but we are missing opportunities to make a profit. Let's consider the 2.4 odd. In this case, the ideal frequency is of course <code>100/2.4 = 41.66%</code> meaning that we expect such event to happen roughly <code>42%</code> of the times we bet on it. As we can see from the blue line, the true frequency is in reality <code>45%</code>. Apparently this may be considered as a positive, because we will win more than what we are expected to, but its important to remember that to bet on a match we need a value threshold to be reached. This means that to bet on a match for which the "correct odd" should be 2.2 (calculated as 100/true frequency, <code>100/45 = 2.2</code>) and our predicted odd is 2.4 we need the bookie to give us a <code>2.4 + 0.1 = 2.5</code> quote, which isn't necessarily something a bookie will do. This means that, albeit not losing money, we won't bet (and won't get a profit) even if the bookie gave us a favourable 2.35 or even 2.45 quote for that specific match.
          </div>
